...
use std::thread;
use common_game::components::resource::{BasicResource, BasicResourceType, Carbon, ComplexResource, ComplexResourceRequest, GenericResource};
...
// this test has no longer sense since our planet is type D
// #[test]
...
#[test]
fn  t10_combination_request()->Result<(),String> {
    println!("+++++ Correct resource request +++++");
    let mut orchestrator = Orchestrator::new()?;
    let mut planet1 = match orchestrator.galaxy_topology.pop(){
        Some(p)=>p,
    ...
    let mut resource1 = None;
    let mut resource2 = None;
        ...
        Explorer { planet_id, orchestrator_channels, explorer_to_planet_channels, planet_to_explorer_channels } => {
                ...
                Some(channels) => {
                    ...
                    match channels.0.recv() {
                        Ok(PlanetToExplorer::GenerateResourceResponse { resource }) => {
                            if resource.is_some() {
                                println!("Explorer received the resource request response correctly.");
                                resource1 = Some(resource.unwrap());
                            } else {
                                println!("Planet gave back None resource.");
                            }
                        }
                ...
                Some(channels) => {
                    ...
                    match channels.0.recv() {
                        Ok(PlanetToExplorer::GenerateResourceResponse { resource }) => {
                            if resource.is_some() {
                                println!("Explorer received the resource request response correctly.");
                                resource2 = Some(resource.unwrap());
                            } else {
                                println!("Planet gave back None resource.");
                            }
                        }
            ...
            match explorer_to_planet_channels {
                ...
                Some(channels) => {
                    ...
                    println!("Sending combination request...");
                    match channels.1.send(ExplorerToPlanet::CombineResourceRequest { explorer_id: 0, msg: ComplexResourceRequest::Diamond(resource1.unwrap().to_carbon()?, resource2.unwrap().to_carbon()?)}) {
                        Ok(_) => { println!("Planet generated combination request."); },
                        Err(err)=>{ panic!("Failed to generate combination request: {}", err); }
                    }
