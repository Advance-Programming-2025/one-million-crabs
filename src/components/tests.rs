...
use crate::{
    Orchestrator,
    components::{CrabRaveConstructor, explorer::BagType},
};
...
fn t01_asteroid_exchange() -> Result<(), String> {
    ...
    {
        Ok(_) => {
            println!("Planet AI started.");
        }
        Err(err) => {
            panic!("Failed to start planet AI: {}", err);
        }
    }
    ...
    match orchestrator
        ...
        .send(OrchestratorToPlanet::Asteroid(
            orchestrator.forge.generate_asteroid(),
        )) {
        Ok(_) => {
            println!("Asteroid sent.");
        }
        Err(err) => {
            panic!("Failed to send asteroid: {}.", err);
        }
    }
            ...
            PlanetToOrchestrator::AsteroidAck { planet_id, rocket } => {
                ...
                if rocket.is_some() {
                        ...
                        .send(OrchestratorToPlanet::KillPlanet)
                    {
                        ...
                        Err(err) => {
                            panic!("Failed to send KillPlanet: {}.", err);
                        }
                    }
                }
            }
...
fn t01_planet_initialization() -> Result<(), String> {
    ...
    let (planet_sender, explorer_receiver): (Sender<PlanetToExplorer>, Receiver<PlanetToExplorer>) =
        unbounded();
    let (explorer_sender, planet_receiver): (Sender<ExplorerToPlanet>, Receiver<ExplorerToPlanet>) =
        unbounded();
